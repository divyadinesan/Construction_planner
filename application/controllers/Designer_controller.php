...
class Designer_controller extends CI_Controller 
{
	...
	public function plan_3D_updation()
	{
		$db_designer_bookingid=$this->input->post('db_designer_bookingid');
		$image=$_FILES['image']['name'];
			if($image!="")
			{
			$time = Time();
			...
			if($this->upload->do_upload('image'))
			{
				$result=array('db_3d_plan'=>$Add_photos);
						$this->load->model('Designer_model');
						$exe=$this->Designer_model->plan_3d_updation_model($result,$db_designer_bookingid);
				if($result)
				{
					echo "<script>alert('Successfully Updated ')</script>";
					$this->threedplan();
