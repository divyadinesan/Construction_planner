...
class Customer_controller extends CI_Controller 
{
	...
	public function architecture_booking_insert()
	{
		...
		$arrdata=array('ab_customer_id'=>$customer_id,'ab_customer_email'=>$customer_email,'ab_architecture_id'=>$architecure_id,'ad_architecture_email'=>$architecture_email,'ab_house_description'=>$customer_description,'ab_booking_status'=>'Pending','ab_payment_status'=>'Pending','house_plan'=>'Pending','house_plan_updation_chat'=>'No Updations Yet');
		$this->load->model('Customer_model');
	...
	public function designer_booking_insert()
	{
		...
		$image=$_FILES['house_plan']['name'];
			...
			{
				$result=array('db_designer_id'=>$designer_id,'db_designer_email'=>$designer_email,'db_customer_id'=>$customer_id,'db_customer_email'=>$customer_email,'db_house_plan'=>$Add_photos,'db_booking_status'=>'Pending','db_payment_status'=>'Pending','db_3d_plan'=>'Pending','house_3d_plan_updation_chat'=>'No Updations Yet');
						$this->load->model('Customer_model');
	...
	public function house_plan_updation_message()
	{
		$architecture_booking_id=$this->input->post('architecture_booking_id');
		$plan_updates=$this->input->post('plan_updates');
		
		$arrdata=array('house_plan_updation_chat'=>$plan_updates);

		$this->load->model('Customer_model');
		$exe=$this->Customer_model->house_plan_updation_message_model($arrdata,$architecture_booking_id);
		if($exe)
		{
			echo "<script>alert('We will update Your Plan soon..')</script>";
			$this->house_plan();
		}
		else
		{
	...
	public function house_3dplan_updation_message()
	{
		$designer_bookingid=$this->input->post('db_designer_bookingid');
		$plan_updates=$this->input->post('3dplan_updates');
		
		$arrdata=array('house_3d_plan_updation_chat'=>$plan_updates);

		$this->load->model('Customer_model');
		$exe=$this->Customer_model->house_3dplan_updation_message_model($arrdata,$designer_bookingid);
		if($exe)
		{
			echo "<script>alert('We will update Your 3DPlan soon..')</script>";
			$this->house_3d_plan();
		}
		else
		{
	...
	public function construction_cost_updation_message()
	{
		$contractor_booking_id=$this->input->post('contractor_booking_id');
		$plan_updates=$this->input->post('construction_cost_updates');
		
		$arrdata=array('contract_details_updation_chat'=>$plan_updates);

		$this->load->model('Customer_model');
		$exe=$this->Customer_model->construction_cost_updation_message_model($arrdata,$contractor_booking_id);
		if($exe)
		{
			echo "<script>alert('We will update You Updated  soon..')</script>";
	...
	public function Architecture_review()
	{
		if($this->session->has_userdata('email'))
			{
				$this->load->model('Customer_model');
				$arrdata['house_plan']=$this->Customer_model->house_plan_fetch_model();
				$this->load->view('Customer/Architecture_review',$arrdata);
			}
			else
			{
				$this->index();
	...
	public function designer_review()
	{
		if($this->session->has_userdata('email'))
			{
				$this->load->model('Customer_model');
				$arrdata['house_3d_plan']=$this->Customer_model->house_3d_plan_model();
				$this->load->view('Customer/designer_review',$arrdata);
			}
			else
			{
				$this->index();
	...
	public function contractor_review()
	{
		if($this->session->has_userdata('email'))
			{
				$this->load->model('Customer_model');
			$arrdata['construction_details']=$this->Customer_model->Construction_cost_details_model();
				...
				$this->load->view('Customer/contractor_review',$arrdata);
			}
			else
			{
				$this->index();
